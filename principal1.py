...
def busqueda_secuencial(idd, v):
    for i in range(len(v)):
        if idd == v[i].identificacion_destinatario:
            return i
    return -1
...
def insercion_ordenada(v, x): #insercion ordenada
    ...
    while izq <= der:
        c = (izq + der) // 2
        if v[c].codigo == x.codigo:
            pos = c
            break
        else:
            if v[c].codigo < v[c].codigo:
                der = c - 1
            else:
                izq = c + 1
    if izq > der:
...
def mayor_monto_por_moneda(envios):
    monedas = ["ARS", "USD", "EUR", "GBP", "JPY"]
    n = len(monedas)
    # matriz de montos máximos
    matriz_montos = [[0] * n for _ in range(n)]
    # matriz de códigos de envío
    matriz_codigos = [[0] * n for _ in range(n)]
    ...
    for envio in envios:
        ...
        if monto_f > matriz_montos[i][j]:
            matriz_montos[i][j] = monto_f
            matriz_codigos[i][j] = envio.codigo
    return matriz_montos, matriz_codigos, monedas


def mostrar_matriz(matriz_codigos, monedas):
    n = len(monedas)
    for i in range(n):
        for j in range(n):
            print(f"Origen {monedas[i]} Destino {monedas[j]}: {round(matriz_codigos[i][j], 2)}")
...
def principal():
    ...
    while op != 0:
        ...
        if op == 1:
            ...
            matriz_montos, matriz_codigos, monedas = mayor_monto_por_moneda(envios)
            mostrar_matriz(matriz_codigos, monedas)
        elif op == 0:
